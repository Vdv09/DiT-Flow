...
class Trainer:
    ...
    def train_one_epoch(self, epoch, num_epochs):
        ...
        for x, y in tqdm(self.train_loader, desc = f"Training Epoch {epoch + 1}/{num_epochs}"):
            ...
            with torch.no_grad():
                latent = self.vae.encode(x, return_sample = False) * self.scale_factor
            ...
            loss = self.diffusion.training_loss(self.model, latent, timesteps, y)
    ...
    def sample(self, epoch):
        ...
        classes = torch.arange(self.num_classes, device = self.device).repeat_interleave(self.num_columns)

        latents = self.diffusion.sample(
            self.ema_model.ema_model,
            image_shape = (self.num_classes * self.num_columns, 4, 8, 8),
            device = self.device,
            y = classes,
        ...
        with torch.no_grad():
            images = self.vae.decode(latents / self.scale_factor)
...
def main(config):
    ...
    vae = class_registry.get_from_registry("vae", **config["vae"]["params"])
    vae.to(device)
    
    checkpoint = torch.load(config["vae"]["checkpoint"], map_location = device)
    ...
    scale_factor = config["vae"]["scale_factor"]
